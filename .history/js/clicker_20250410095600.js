document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        if (!upgradeData || !Array.isArray(upgradeData) || upgradeData.length === 0) {
            ...
            console.log('Détails des upgrades dans upgradeData :', JSON.stringify(upgradeData, null, 2));
    ...
    // Charger les améliorations (style : icône + prix à côté, tooltip au survol, disparition après achat)
    if (upgrades.length === 0) {
        console.warn('Aucun upgrade à afficher (tous déjà achetés ou liste vide).');
        upgradeContainer.innerHTML = '<p style="color: white;">Aucun upgrade disponible.</p>';
    } else {
        upgrades.forEach((upgrade, index) => {
            ...
            console.log(`Affichage de l'upgrade "${name}" avec un coût de ${cost}`);

            upgradeItem.innerHTML = `
                ${iconOrImage}
                <span class="upgrade-price">${cost}</span>
                <div class="tooltip">

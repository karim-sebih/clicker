...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!Array.isArray(shopData)) {
        console.error('shopData n\'est pas un tableau valide:', shopData);
        return;
    }
    if (!Array.isArray(upgradeData)) {
        console.error('upgradeData n\'est pas un tableau valide:', upgradeData);
        return;
    }
    ...
    upgradeData.forEach((upgrade, index) => {
        ...
        upgradeItem.innerHTML = `
            ...
            <div class="upgrade-info">
                ...
                <span class="upgrade-cost" id="priceupgrade${index}">${upgrade.cost} ressources</span>
            </div>
            <div class="upgrade-count" id="ownedupgrade${index}">${upgrade.owned}</div>
    ...
    async function getShopData() {
        const res = await fetch('./json/shop.json'); // Ajustez le chemin ici
        if (!res.ok) {
    ...
    async function getUpgradeData() {
        const res = await fetch('./json/upgrade.json'); // Ajustez le chemin ici
        if (!res.ok) {

document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Forcer la cliquabilité de #sectionRight
    sectionRight.style.pointerEvents = 'auto';
    sectionRight.style.zIndex = '100';
    sectionRight.style.position = 'relative';
    ...
    function loadUpgrades() {
        ...
        console.log('Upgrades disponibles:', availableUpgrades);
        ...
        if (availableUpgrades.length === 0) {
            ...
            availableUpgrades.forEach((upgrade, index) => {
                ...
                if (upgrade.image && upgrade.image.src) {
                    ...
                    iconElement.style.width = '100%'; // Occuper tout l'espace du parent
                    iconElement.style.height = '100%';
                    iconElement.style.objectFit = 'contain';
                    // Log pour vérifier le rendu
                    iconElement.onload = () => console.log(`Icône chargée: ${name}, src: ${upgrade.image.src}`);
                    iconElement.onerror = () => console.error(`Erreur de chargement de l'icône: ${name}, src: ${upgrade.image.src}`);
                } else {
                    ...
                    iconElement.style.lineHeight = '40px'; // Centrer verticalement
    ...
    // Gestionnaire sur #sectionRight
    sectionRight.addEventListener('click', (e) => {
        console.log('Clic détecté sur sectionRight, cible:', e.target);
    });

    // Gestionnaire global sur window
    window.addEventListener('click', (e) => {
        console.log('Clic global (window) détecté, cible:', e.target);
    });

    // Gestionnaire global sur document
    document.addEventListener('click', (e) => {
        console.log('Clic global (document) détecté, cible:', e.target);
    });

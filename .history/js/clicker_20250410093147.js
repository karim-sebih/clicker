...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    upgrades.forEach((upgrade, index) => {
        ...
        let iconOrImage;
            if (upgrade.image && upgrade.image.src) {
                iconOrImage = `<img src="${upgrade.image.src}" `;
            } else {
                iconOrImage = `<i class="${upgrade.icon}"></i>`;
            }

        // Afficher uniquement l'image/icône avec un tooltip contenant les détails
        upgradeItem.innerHTML = `
            ${iconOrImage}
            <div class="tooltip">
                ${upgrade.image} - ${upgrade.cost} ressources <hr></br>
                ${upgrade.description}
            </div>
            
            <span class="upgrade-cost hidden" id="priceupgrade${index}">${upgrade.cost}</span>
            <span class="upgrade-count hidden" id="ownedupgrade${index}">${upgrade.owned}</span>
        `;
    ...
    buildings.forEach((building, index) => {
        ...
        buildingItem.innerHTML = `
            ...
            <div class="building-count" id="ownedbuilding${index}">${building.count}</div>
        `;
    ...
    async function getShopData() {
        const res = await fetch('./json/shop.json'); // Ajustez le chemin si nécessaire
        if (!res.ok) {
    ...
    async function getUpgradeData() {
        const res = await fetch('./json/upgrade.json'); // Ajustez le chemin si nécessaire
        if (!res.ok) {

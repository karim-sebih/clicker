document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Variables de jeu avec valeurs par défaut
    ...
    // Copies originales pour éviter les modifications directes
    ...
    function loadUpgrades() {
        ...
        const availableUpgrades = upgrades.filter(upgrade => upgrade.owned === 0);
        ...
        availableUpgrades.forEach((upgrade, index) => {
            ...
            upgradeItem.addEventListener('click', () => buyUpgrade(upgrade, index));
    ...
    clicker.addEventListener('click', function() {
        pts += cpc;
        updateDisplay();
        loadUpgrades(); // Recharger pour afficher les upgrades abordables
        saveGame();
    });
    ...
    function buyUpgrade(upgrade, index) {
        if (pts >= upgrade.cost) {
            ...
            const upgradeIndex = upgrades.findIndex(u => u.name === upgrade.name);
            if (upgradeIndex !== -1) {
                upgrades[upgradeIndex].owned = 1;
            }
